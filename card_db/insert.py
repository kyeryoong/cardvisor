...
def values_main(my_port, my_password) :
    ...
    for i in temp :
        ...
        sql = ("INSERT INTO `main` VALUES(" + pre_sql + "');")
...
def values_benefits(my_port, my_password)  :
    conn = methods.connect_db(my_port, my_password)
    cur = conn.cursor()

    file = open("benefits.txt", "r", encoding="utf-8")
    elements = file.readlines()
    file.close()
        ...
        for j in range(1, len(temp)) :
            ...
            if parsed_temp != None :
                if parsed_temp[0] in brands_dictionary :
                    sql = ("INSERT INTO `" + brands_dictionary.get(parsed_temp[0]) + "` VALUES ('" 
